    ...
    },
    {
        ...
        "changes": [
            ...
            },
            {
                ...
                "entity": "Preference",
                "description": "No longer deduplicate - keep all entries for evolution tracking",
            },
        ],
    },
    {
        ...
        "changes": [
            {
                "type": "schema_change",
                "description": "Replaced rigid types with generic Fact",
            },
            {
                "type": "behavior_change",
                "description": "Facts deduplicated by (type, subject, value)",
            },
        ],
    },
    {
        ...
        "changes": [
            {
                "type": "behavior_change",
                "description": "LLM merges existing facts with new extractions",
            },
            {
                "type": "node_removed",
                "description": "Removed deduplicate_memories_node",
            },
        ],
    },
]
...
PII_PATTERNS: List[Dict[str, Any]] = [
    # Social Security Numbers (XXX-XX-XXXX, XXXXXXXXX, XXX XX XXXX)
    {
        ...
        "validator": lambda m: not m.group(1)
        .replace("-", "")
        .replace(" ", "")
        .startswith("000"),
    },
    # Credit Card Numbers (major brands, with or without separators)
    {
        ...
        "pattern": re.compile(r"\b([3-6]\d{3}[-\s]?\d{4}[-\s]?\d{4}[-\s]?\d{3,4})\b"),
    },
...
def scrub_pii(
    text: str, replacement: str = "[REDACTED]", patterns: Optional[List[str]] = None
) -> str:
    """
...
class Filter:
    class Valves(BaseModel):
        ...
        postgres_database: str = Field(
            default="langgraph_memory", description="PostgreSQL database name"
        )
        postgres_user: str = Field(
            default="langgraph", description="PostgreSQL username"
        )
        ...
        extraction_model_max_tokens: int = Field(
            default=1000, description="Max tokens for extraction model response"
        )
        ...
        show_status: bool = Field(
            default=True, description="Show memory processing status messages"
        )
        show_injected_memories: bool = Field(
            default=False, description="Show which memories were injected (debug mode)"
        )
        ...
        relevance_model_max_tokens: int = Field(
            default=1000, description="Max tokens for relevance model response"
        )
    ...
    class UserValves(BaseModel):
        ...
        enabled: bool = Field(
            default=True, description="Enable LangGraph memory for this user"
        )
        show_status: bool = Field(
            default=True, description="Show status messages for this user"
        )
    ...
    async def _resolve_user(self, __user__: Optional[Dict[str, Any]]):
        """
        Resolve the __user__ dict into a real UserModel.

        generate_chat_completion() and its downstream cache key builders use
        attribute access (user.id, user.role) — e.g. openai.get_all_models is
        decorated with key_builder=lambda ...: f"openai_all_models_{user.id}".
        Passing the raw __user__ dict raises:
        ...
        try:
            ...
            # Users.get_user_by_id is async in Open WebUI >= 0.11, sync before that
            if asyncio.iscoroutine(result):
                result = await result
            if result is None:
                self._log(f"UserModel not found for {user_id[:8]}...", "error")
            return result
        except Exception as e:
            self._log(
                f"Failed to resolve UserModel for {user_id[:8]}...: {type(e).__name__}: {e}",
                "error",
            )
            return None
    ...
    async def _initialize_graph(self):
        ...
        try:
            ...
            self._log(
                f"Connecting to PostgreSQL at {self.valves.postgres_host}:{self.valves.postgres_port}"
            )
                ...
                with psycopg.connect(conn_string, autocommit=True) as setup_conn:
                    ...
                    with setup_conn.cursor() as cur:
                        ...
                        if current_version < SCHEMA_VERSION:
                            self._log(
                                f"Schema upgrade needed: v{current_version} → v{SCHEMA_VERSION}",
                                "info",
                            )
                            ...
                            self._log(
                                f"Schema is current (v{current_version})", "debug"
                            )
            ...
            self._log(
                f"Failed to import LangGraph dependencies: {e}. "
    ...
    def _process_merged_facts_node(self, state: MemoryGraphState) -> MemoryGraphState:
        ...
        try:
            ...
            for fact in extraction.facts:
                ...
                if not fact.get("type") or not fact.get("subject"):
                    self._log(
                        f"Skipping invalid fact (missing type/subject): {fact}",
                        "warning",
                    )
                    continue
            ...
            self._log(
                f"Failed to process merged data: {type(e).__name__}: {e}", "error"
            )
            import traceback
        ...
        self._log(
            f"=== PROCESS_MERGED NODE COMPLETE: {len(state.get('facts', []))} facts ===",
            "info",
        )
    ...
    def _create_summary_node(self, state: MemoryGraphState) -> MemoryGraphState:
        ...
        if "ownership" in facts_by_type:
            own_texts = [
                f"{f['subject']}: {f['value']}" for f in facts_by_type["ownership"]
            ]
        ...
        if "relationship" in facts_by_type:
            rel_texts = [
                f"{f['subject']}: {f['value']}" for f in facts_by_type["relationship"]
            ]
        ...
        if "skill" in facts_by_type:
            skill_texts = [
                f"{f['subject']}: {f['value']}" for f in facts_by_type["skill"]
            ]
        ...
        if "event" in facts_by_type:
            event_texts = [
                f"{f['subject']}: {f['value']}" for f in facts_by_type["event"]
            ]
    ...
    async def _call_extraction_model(
        ...
        try:
            ...
            if user is None:
                self._log(
                    "WARNING: user is None - generate_chat_completion may fail!",
                    "error",
                )
            if request is None:
                self._log(
                    "WARNING: request is None - generate_chat_completion may fail!",
                    "error",
                )
            ...
            self._log(
                f"Extraction payload: model={payload['model']}, prompt_len={len(prompt)}",
                "info",
            )
            ...
            self._log(
                f"generate_chat_completion returned type: {type(response)}", "info"
            )
            ...
            if isinstance(response, dict):
                ...
                if not choices:
                    self._log(
                        f"No choices in extraction model response. Full response: {response}",
                        "error",
                    )
                    return None
                ...
                self._log(
                    f"Extraction model response length: {len(response_text)} chars",
                    "debug",
                )
                return self._clean_model_response(response_text)
            else:
                self._log(
                    f"Unexpected response type from extraction model: {type(response)}. Response: {response}",
                    "error",
                )
                return None
    ...
    async def _get_user_memory_state(
        ...
        try:
            ...
            snapshot = await asyncio.wait_for(
                loop.run_in_executor(None, lambda: self.memory_graph.get_state(config)),
                timeout=10.0,  # 10 second timeout
            )

            if snapshot and snapshot.values:
                self._log(
                    f"Found existing memory state with {snapshot.values.get('total_facts', 0)} facts",
                    "debug",
                )
            ...
            self._log(
                f"Initializing new memory state for user {user_id[:8]}...", "debug"
            )
            return {
                "user_id": user_id,
            ...
            self._log(
                f"Failed to retrieve memory state: {type(e).__name__}: {e}", "error"
            )
            import traceback
    ...
    async def _update_user_memory_state(
        ...
        try:
            ...
            # =====================================================================
            self._log(
                f"Step 1: Merging {len(existing_facts)} existing facts with {len(new_messages)} new messages...",
                "info",
            )

            # Format existing facts for prompt
            existing_facts_json = (
                json.dumps(existing_facts, indent=2) if existing_facts else "[]"
            )
            ...
            if self.valves.pii_filter_enabled and self.valves.pii_scrub_input:
                self._log(
                    "PII pre-scrub: scanning messages before extraction...", "info"
                )
...
{json.dumps(messages_for_extraction, indent=2)}"""
            ...
            merged_json = await self._call_extraction_model(
                merge_prompt, user=user, request=request
            )

            self._log(
                f"Merge model returned: {merged_json[:500] if merged_json else 'NONE/EMPTY'}",
                "info",
            )
            ...
            if merged_json:
                ...
                try:
                    ...
                    self._log(
                        f"Parsed merged data: {len(merged_data.get('facts', []))} facts",
                        "info",
                    )
                ...
                self._log(
                    "Merge model returned empty - keeping existing facts", "warning"
                )
            ...
            if self.valves.pii_filter_enabled and new_facts:
                ...
                if blocked_count > 0:
                    self._log(
                        f"PII post-validation: blocked {blocked_count} fact(s) containing PII",
                        "warning",
                    )
                else:
            ...
            if facts_equal(existing_facts, new_facts):
                self._log(
                    f"No changes to facts ({len(existing_facts)} facts unchanged) - skipping graph invoke",
                    "info",
                )
                return  # Skip checkpoint creation when nothing changed

            self._log(
                f"Facts changed: {len(existing_facts)} → {len(new_facts)} - invoking graph",
                "info",
            )
            ...
            result = await asyncio.wait_for(
                loop.run_in_executor(
                    None, lambda: self.memory_graph.invoke(current_state, config)
                ),
            ...
            self._log(
                f"Memory updated for user {user_id[:8]}: {result.get('total_facts', 0)} facts stored",
                "info",
            )
            ...
            self._log(
                f"Failed to update memory state: {type(e).__name__}: {e}", "error"
            )
            import traceback
    ...
    async def _select_relevant_memories(
        ...
        always_facts = [
            f
            for f in facts
            if f.get("type", "other") in self.valves.always_inject_types
        ]
        filterable_facts = [
            f
            for f in facts
            if f.get("type", "other") not in self.valves.always_inject_types
        ]

        if not filterable_facts:
            self._log(
                "All facts are always-inject types, skipping relevance filter", "debug"
            )
            return always_facts
...
{json.dumps(recent_conversation, indent=2)}"""
        ...
        try:
            ...
            if not response_text:
                self._log(
                    "Relevance model returned empty — falling back to all facts",
                    "warning",
                )
                return facts
            ...
            self._log(
                f"Relevance model returned invalid JSON: {e} — falling back to all facts",
                "warning",
            )
            return facts
        except Exception as e:
            self._log(
                f"Relevance filtering failed: {type(e).__name__}: {e} — falling back to all facts",
                "warning",
            )
            return facts
...
{memory_state.get('memory_summary', '')}"""
    ...
    async def inlet(
        ...
        self._log(
            f"Processing user={user_id[:8]}... chat={conversation_id[:8] if conversation_id else 'default'}...",
            "info",
        )

        # generate_chat_completion() needs a UserModel, not the __user__ dict
        user_obj = await self._resolve_user(__user__)
        ...
        try:
            ...
            self._log(
                f"Got memory state with {memory_state.get('total_facts', 0)} facts",
                "info",
            )
                    ...
                    if messages:
                        ...
                        if self.valves.show_status and __event_emitter__:
                            ...
                            if injected_count > 0:
                                if (
                                    self.valves.relevance_filter_enabled
                                    and injected_count < total_stored
                                ):
            ...
            self._log(
                f"Extraction check: {len(user_messages)} user messages, threshold={self.valves.extraction_threshold}",
                "info",
            )
            ...
            if len(user_messages) >= self.valves.extraction_threshold:
                ...
                try:
                    ...
                    self._log(
                        f"Memory update FAILED: {type(update_err).__name__}: {update_err}",
                        "error",
                    )
                    import traceback

                    self._log(
                        f"Update error traceback: {traceback.format_exc()}", "error"
                    )
                ...
                self._log(
                    f"Threshold NOT met: {len(user_messages)} < {self.valves.extraction_threshold}",
                    "info",
                )
